...
It can be necessary to use an `id` different from the the hash for the element to scroll to (exemple when using [fullPage](https://github.com/alvarotrigo/fullPage.js/) as there is a [bug](https://github.com/alvarotrigo/fullPage.js/issues/72) when `id`s have the same values than anchors). To do so you can use the `prefix` prop a such :
